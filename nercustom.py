...
this code focuses on annoing data for single entity name
'''
...
class StringOperations:
    ...
    # user defined function to remove gaps between the strings
    ...
    # user defined function for splitting string as individual words
    ...
    # user defied function to convert first letter as capital
    ...
    # user defined function to add prefixes to the string
...
class Generate:
    ...
    def generate_better_characters(self):
        return [
            ...
            for message in self.df["text"].str.lower()]
...
# use this technique when you have to train only on single Entity
# value

class GeneratePatterns:
    def __init__(self, file,add_prefix,split_str):
        
        self.gd = GenerateData(file)
        self.savefile = GetData()
        self.final_list = sorted(self.gd.finalgenerateddata(add_prefix,split_str))
        self.patterns = []
        
    # user defined function for creating training corpus
    def create_training_data(self, type):
        try:
    ...
    def savedata(self, file_name, type):
        try:
            datapatterns = self.create_training_data(type)
...
class GenerateRules:
    ...
    def __init__(self, jsonfile, type, generatefilejson,prefix,split_words):

       
        self.pat = GeneratePatterns(file=jsonfile,add_prefix=prefix,split_str=split_words)
        self.pattern_data = self.pat.create_training_data(type)
        self.pat.savedata(generatefilejson, type)
        self.nlp = English()
        self.results = []
...
class GenerateTrainingData:
    ...
    # it is better to test our model on some psuedo real data
